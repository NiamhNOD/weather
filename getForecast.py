...
def getForecastCork(darkskyKey):
    ...
    weather_cork_response = requests.get('https://api.darksky.net/forecast/' + darkskyKey + '/51.8498,-8.3959,' + str(int(seconds)) + '?exclude=currently,hourly,minutely,alerts,flags&units=uk2')
    ...
    c.execute("INSERT INTO weatherDaily VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", (localTime, summary, icon, sunriseTime, sunsetTime, precipIn, precipProb, tempHigh, tempLow, hum, press, windS, windB, cloudC))
    conn.commit()
    conn.close()
